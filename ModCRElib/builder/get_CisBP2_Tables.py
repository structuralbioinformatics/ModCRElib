"""
Rebuild CIS-BP v2 SQL tables aligned to ModCRE-required schema subsets.
"""
...
def parse_options():
    """
    Parse CLI options for CIS-BP2 table extraction and rewriting.

    Returns:
        optparse.Values: Namespace with SQL source, TF/protein info tables, and output root.
    """
...
def read_SQL_Tables(sql_file):
    """
    Parse raw SQL INSERT blocks into a table-indexed in-memory representation.

    Args:
        sql_file (str): Path to the CIS-BP SQL dump.

    Returns:
        dict: Mapping ``table_name -> set(tuple((column, value), ...))``.
    """
    sql=open(sql_file,"r")
    table_name=None
    db_table={}
...
def write_tf_table(rootname,table_name,table_keys,tf_info,prot_info,db_table):
    """
    Write the normalized TF table SQL output file.

    Args:
        rootname (str): Output file prefix.
        table_name (str): SQL table name to emit.
        table_keys (list[str]): Ordered target columns.
        tf_info (dict): Parsed TF metadata records.
        prot_info (dict): Parsed protein metadata records.
        db_table (dict): Parsed SQL source tables.
...
def write_table(rootname,table_name,table_keys,tf_info,prot_info,db_table):
    """
    Write one normalized SQL table from merged CIS-BP sources.
...
def main():
    """
    Build normalized CIS-BP2 SQL outputs required by ModCRE.

    Workflow:
        1. Define target schemas.
        2. Parse options and load SQL/TSV sources.
        3. Build optional TF selection subset.
        4. Merge source records and emit final SQL tables.

    Returns:
        None.
    """
    # Step 1) Define target SQL schemas required by ModCRE.
    cisbp_tables={}
    cisbp_tf_table={}
    ...
    # Step 2) Parse options and read raw SQL insert data.
    options = parse_options()
    db_table= read_SQL_Tables(os.path.abspath(options.sql_file))
    # Step 3) Optionally load a TF selection whitelist.
    use_selection=False
    ...
    # Step 5) Load protein sequence metadata TSV.
    titles=[]
    #print "PROTEIN"
    prot_info={}
    ...
    #print prot_info
    # Step 6) Write normalized output SQL tables.

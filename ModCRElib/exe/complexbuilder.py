...
def make_dimers_list(index,namemap,filelist,added_proteins,selected,conformers,dimers,args):
      ...
      for file in os.listdir(args["folder"]):
        ...
        if m:
            ...
            mm=re.search("(\S+).(\S+)_(\S+).(\d+)-(\d+).pdb",file)
            if mm:
                start_model = int(mm.group(4))
                end_model   = int(mm.group(5))
                prot        = mm.group(1)
                pdb         = mm.group(2)
                chain       = mm.group(3)
                binding     = "%d-%d"%(int(mm.group(4)),int(mm.group(5)))
                pdb_chain   = pdb+"_"+chain
                code_argument = prot+"."+pdb+"_"+chain+"."+binding
                conformers.setdefault((prot,pdb,binding),set()).add(chain)
                modelsize.setdefault((prot,pdb,binding),[]).append(end_model-start_model)
    ...
    for chain in structure_1: 
        for chain_2 in structure_2[0]: 
            if is_dna(chain) and is_dna(chain_2): # If one chain is DNA, use the positions.
              ...
              residues_id = [residue.get_id()[1] for residue in chain_residues]
              min_id, max_id = residues_id[0], residues_id[-1]
              reversed_positions = []
              for a, b in testing_positions:
                  rev = (min_id + max_id - b, min_id + max_id - a)
                  if rev != (a, b):
                      reversed_positions.append(rev)
              for check_positions in [testing_positions, reversed_positions]:
                for a,b in check_positions:
                  ...
                  difference = b - a
                  residues = chain_residues[starting_index: starting_index + difference + 1]

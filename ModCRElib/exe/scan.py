...
from ModCRElib.profile.run_utils import append_file
...
for fasta_sequence in functions.parse_fasta_file(input_file):
    ...
    print(f"\t{dna_header} => sequence: {dna_sequence}")
    if ":" in dna_header:
        header_range = dna_header.split(":")[-1].split("-")
        if len(header_range) >= 2 and header_range[0].isdigit():
            if int(header_range[0]) == int(options.index):
                start_index = 0
    ...
    if not fileExist(database_extended_file):
        ...
        for pwm_file in os.listdir(os.path.join(pbm_dir, "pwms")):
            if pwm_file.endswith("meme"): 
               ...
               if m:
                  append_file(os.path.join(pbm_dir, "pwms", pwm_file), database_extended_file)
    else:
        ...
        for pwm_file in os.listdir(os.path.join(pbm_dir, "pwms")):  
            if pwm_file.endswith("meme"): 
               ...
               if m: 
                 ...
                 if len(chains) == 1 and pdb_code in tf_codes:
                     append_file(os.path.join(pbm_dir, "pwms", pwm_file), database_file)
 ...
 for cluster in clusters:
    ...
    for members in cluster['members']:
        ...
        for i in range(2):
            ...
            if verbose: sys.stdout.write(f"\t\t--Member {i} {members[i].get_hit()} {members[i].get_start()} {members[i].get_end()}..\n")
            if external is not None:
            ...
            for ortholog in ortholog_hits.get_hits((0,None)):
                ...
                if os.path.exists(threading_file):
                    ...
                    iface_len = interface_obj.get_interface_length()
                    dna_start = max(0, members[i].get_start() - 1)
                    # Input-DNA window at the FIMO hit (motif + downstream bases).
                    ref_iface = dna_sequence[dna_start:dna_start + iface_len]
                    if len(ref_iface) < iface_len:
                        ref_iface += "N" * (iface_len - len(ref_iface))
                    if len(dna_seq_dummy) < iface_len:
                        dna_seq_interface = dna_seq_dummy + "N" * (iface_len - len(dna_seq_dummy))
                        # Same logic as threader.py: motif + continuation from the binding-site window.
                        dna_seq_extended = dna_seq_dummy + ref_iface[len(dna_seq_dummy):]
                        if len(dna_seq_extended) < iface_len:
                            dna_seq_extended += "N" * (iface_len - len(dna_seq_extended))
                    elif len(dna_seq_dummy) > iface_len:
                        dna_seq_interface = dna_seq_dummy
                        dna_seq_extended = ref_iface
                    else:
                        dna_seq_interface = dna_seq_dummy
                        dna_seq_extended = ref_iface if ref_iface else dna_seq_dummy
                    if not dna_seq_extended:
                        dna_seq_extended = dna_seq_interface
                    dna_seq_interface_complementary = triads.get_complementary_dna_sequence(dna_seq_interface)

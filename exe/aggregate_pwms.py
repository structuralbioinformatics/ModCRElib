"""
Aggregate related PWMs from JSON manifests into aligned cluster-level motifs.

This module resolves motif files from multiple databases, compares them with
TOMTOM, clusters similar PWMs, aligns cluster members as tensors, and writes
aggregate motifs as well as optional reference-based realignments. Several
other scripts import helpers from here for tensor conversion and clustering.
"""
...
def parse_options():
    """
    Parse the command line options for PWM aggregation.

    Returns:
        optparse.Values: Parsed CLI options describing the JSON input,
        clustering/alignment settings, database locations, and output paths.
    """
...
def is_padding(column, threshold=0.0001):
    """
    Determine whether a profile column should be treated as padding.

    Args:
        column (np.ndarray): One PWM/profile column.
        threshold (float): Columns with summed signal below this value are
            considered padding.

    Returns:
        bool: True when the column is effectively empty.
    """
...
def get_trim(x, y):
    """
    Trim one tensor according to the non-padding span of another tensor.

    Args:
        x (np.ndarray): Array to trim, with shape ``(B, L)``.
        y (np.ndarray): Reference array whose nonzero support defines the kept
            interval, also with shape ``(B, L)``.

    Returns:
        np.ndarray: Slice of ``x`` spanning only the columns where ``y`` has
        nonzero signal.
    """
...
def padding2D(arr,final_length=50,mode="right"):
    """
    Pad or crop a 2D array along the length dimension.
...
def pearson_correlation(a, b,gap_penalty,threshold=0.0001):
    """
    Calculate Pearson correlation between two PWM columns.

    Args:
        a, b (np.ndarray): PWM columns to compare.
        gap_penalty (float): Penalty returned when one of the columns is padding.
        threshold (float): Minimum summed signal to consider a column non-padding.

    Returns:
        float: Pearson correlation, or ``-gap_penalty`` for padding columns.
    """
...
def product(a,b,gap_penalty,threshold=0.0001):
    """
    Calculate the dot product between two PWM columns.
    ...
    Returns:
        float: Dot product, or ``-gap_penalty`` for padding columns.
    """
...
def kullback_leibler(a,b,gap_penalty,threshold=0.0001):
    """
    Calculate the symmetric Kullback-Leibler divergence between two PWM columns.
    ...
    Returns:
        float: Symmetric KL divergence, or ``-gap_penalty`` for padding columns.
    """
    epsilon = 1.0e-12
    if is_padding(a, threshold) or is_padding(b, threshold):
       return  -gap_penalty
...
def jensen_shanon(a,b,gap_penalty,threshold=0.0001):
    """
    Calculate Jensen-Shannon divergence between two PWM columns.
    ...
    Returns:
        float: Jensen-Shannon divergence, or ``-gap_penalty`` for padding columns.
    """
    epsilon = 1.0e-12
    if is_padding(a, threshold) or is_padding(b, threshold):
       return  -gap_penalty
...
def get_metric(choice):
    """
    Select the profile-comparison metric used during tensor alignment.

    Args:
        choice (str): Metric name. Supported values are ``pearson``,
            ``kullback_leibler``, ``jensen_shanon``, and ``product``.

    Returns:
        callable: Function that scores a pair of PWM columns.
    """
    if choice == "pearson":
...
def align_without_gaps(profile1, profile2, metric, gap_penalty=0.5, threshold=0.0001):
    ...
    Args:
        profile1 (np.ndarray): First profile of shape ``(4, L1)``.
        profile2 (np.ndarray): Second profile of shape ``(4, L2)``.
        metric (callable): Column-comparison function such as
            :func:`product` or :func:`pearson_correlation`.
        gap_penalty (float): Penalty passed to ``metric`` for padding columns.
        threshold (float): Minimum sum to be considered non-padding.

    Returns:
        tuple: ``(max_score, max_offset, alignment)`` where ``alignment`` is a
        list of matched column-index pairs ``(i, j)``.
    """
    L1 = profile1.shape[1]
    L2 = profile2.shape[1]
    max_score = float('-inf')
...
def build_aligned_profile(profile1, profile2, offset):
    ...
    Args:
        profile1 (np.ndarray): Reference profile with shape ``(4, L1)``.
        profile2 (np.ndarray): Profile to realign, with shape ``(4, L2)``.
        offset (int): Offset returned by :func:`align_without_gaps`.

    Returns:
        np.ndarray: Array with shape ``(4, L1)`` containing the aligned version
        of ``profile2`` padded with zeros where needed.
    """
    L1 = profile1.shape[1]
    L2 = profile2.shape[1]
...
def get_pwms(json_file,jaspar,cisbp,hocomoco,modcre,verbose=False,selected_path=None):
    """
    Resolve PWM files listed in a JSON manifest.

    Args:
        json_file (str): JSON file containing motif metadata.
        jaspar, cisbp, hocomoco, modcre (str | None): Base directories for the
            supported PWM databases.
        verbose (bool): If True, print progress and skip reasons.
        selected_path (str | None): Optional explicit base directory overriding
            the database-specific path from the JSON metadata.

    Returns:
        list[str]: Paths to PWM files that exist and can be parsed as non-empty motifs.
        If the manifest is very large, the returned list may be downsampled.
    """
    pwms=[]
    models={}
...
def get_reference(json_file,jaspar,cisbp,hocomoco,modcre,ratio, verbose=False,selected_path=None):
    """
    Select reference PWMs from a JSON manifest based on similarity metadata.

    Only experimental-database motifs whose stored ``similarity`` field is at
    least ``ratio`` are retained.

    Args:
        json_file (str): JSON file containing motif metadata.
        jaspar, cisbp, hocomoco, modcre (str | None): Base directories for the
            supported PWM databases.
        ratio (float): Minimum similarity required to keep a motif as reference.
        verbose (bool): If True, print progress and skip reasons.
        selected_path (str | None): Optional explicit base directory overriding
            the database-specific path from the JSON metadata.

    Returns:
        list[str]: Paths to reference PWM files.
    """
    pwms=[]
    with open(json_file,"r") as json_fp:
         json_obj=json.load(json_fp)
...
def pwm2tensor(msa_obj):
    """
    Convert a DNA PWM object into a tensor-like NumPy array.

    Args:
        msa_obj (PWM.nMSA): Motif object containing a PWM.

    Returns:
        np.ndarray: Float array with shape ``(4, L)``.
    """
...
def tensor2msa(tensor,name="MOTIF",sequences=False):
    ...
    #with numpy
    """
    Convert a tensor-like PWM representation back into an :class:`PWM.nMSA`.

    Args:
        tensor (np.ndarray): PWM array with shape ``(4, L)``.
        name (str): Motif name assigned to the returned object.
        sequences (bool): If True, synthesize sequences from the PWM after
            storing and enhancing it.

    Returns:
        PWM.nMSA: Motif object built from the tensor.
    """
...
def makedb(pwms,folder):
    """
    Concatenate multiple PWM files into a temporary TOMTOM database.

    Args:
        pwms (list[str]): PWM file paths to concatenate.
        folder (str): Directory where the temporary database file is created.

    Returns:
        str: Path to the generated TOMTOM database file containing the
        concatenated motif definitions.
    """
    name=""
    for pwm in pwms:
...
def get_clusters(matrix,pvalue_threshold=0.005, distance_threshold=0.20):
    """
    Cluster PWMs from a TOMTOM-derived distance matrix.

    Args:
        matrix (np.ndarray): Pairwise distance/similarity matrix.
        pvalue_threshold (float): Values at or below this threshold are forced
            to zero before clustering.
        distance_threshold (float): Agglomerative clustering cut-off.

    Returns:
        dict: Mapping ``cluster_center_index -> member_indices`` where the key
        is chosen as the member with the smallest summed in-cluster distance.
    """
...
def reverse_tensor(tensor,motif_name):
    """
    Reverse-complement a PWM tensor through :class:`PWM.nMSA`.

    Args:
        tensor (np.ndarray): PWM tensor with shape ``(4, L)``.
        motif_name (str): Temporary motif name used during conversion.

    Returns:
        np.ndarray: Reverse-complemented tensor with shape ``(4, L)`` obtained
        by converting through :class:`PWM.nMSA`.
    """
...
def main():
    """
    Run the command-line PWM aggregation workflow.

    Workflow:
        1. Resolve PWM files from one or more JSON manifests.
        2. Compare motifs with TOMTOM and cluster them by pairwise similarity.
        3. Align cluster members as tensors and build cluster-average motifs.
        4. Optionally realign clusters against selected reference PWMs.
        5. Write aggregated motifs, member motifs, logos, and bookkeeping files.

    Returns:
        None. Output directories, motifs, logos, and log files are written to disk.
    """
    # Arguments & Options #
